 ...
 */
public abstract class TinyUIContainerScreen<T extends AbstractContainerMenu> extends AbstractContainerScreen<T> {
    ...
    public TinyUIContainerScreen(T menu, Inventory playerInventory, Component title) {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        UIRenderContext context = new GuiRenderContext(graphics, Minecraft.getInstance().font);
    ...
    private void updateSlotPositions() {
        ...
        for (UISlot uiSlot : uiSlots) {
            ...
            if (index >= 0 && index < this.menu.slots.size()) {
                ...
                // 检查是否在屏幕内 (处理滚动裁剪)
                if (uiSlot.isEnabledAndVisible() && isVisibleInHierarchy(uiSlot)) {
                     setSlotPosition(mcSlot, absX - guiLeft, absY - guiTop);
                }
            }
        }
    }
