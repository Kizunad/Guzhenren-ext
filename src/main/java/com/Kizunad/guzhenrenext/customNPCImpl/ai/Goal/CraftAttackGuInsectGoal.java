...
/**
 * 制作攻击型蛊虫：根据 NPC 当前转数，自动制作对应标签下的蛊虫，保持最低储备量。
 */
public class CraftAttackGuInsectGoal extends AbstractGuzhenrenGoal {
    ...
    public CraftAttackGuInsectGoal() {
        super("craft_attack_gu_insect");
    }
    ...
    public void start(INpcMind mind, LivingEntity entity) {
        ...
        if (!canStore(mind, option.item())) {
            ...
            MindLog.decision(
                MindLogLevel.WARN,
                "CraftAttackGuInsectGoal 背包已满，跳过制作蛊虫: {}",
        ...
        if (material < option.cost()) {
            ...
            MindLog.decision(
                MindLogLevel.WARN,
                "CraftAttackGuInsectGoal 材料不足，需 {} 点，当前 {}",
                option.cost(),
                material
            );
            return;
        ...
        if (!remaining.isEmpty()) {
            ...
            MindLog.execution(
                MindLogLevel.WARN,
                "CraftAttackGuInsectGoal 背包已满，蛊虫已掉落: {}",
        ...
        MindLog.decision(
            MindLogLevel.INFO,
            "CraftAttackGuInsectGoal 制作蛊虫成功: {}，消耗 {} 材料，剩余 {}，标签 {}",
            BuiltInRegistries.ITEM.getKey(option.item()),
            option.cost(),
            npc.getMaterial(),
            option.tag().location()
    ...
    private boolean hasEnoughStock(INpcMind mind) {
        ...
        for (int i = 0; i < mind.getInventory().getMainSize(); i++) {
            ...
            if (GuInsectUtil.isAttackGu(stack)) {
                count++;
                if (count >= MIN_ATTACK_GU_STOCK) {
                    return true;
                }
            }
        }
    ...
    private CraftOption findCraftOption(
        ...
        if (candidates.isEmpty()) {
            MindLog.decision(
                MindLogLevel.WARN,
                "CraftAttackGuInsectGoal 未找到标签 {} 对应物品",
                tag.location()
            );
            return null;
        }
