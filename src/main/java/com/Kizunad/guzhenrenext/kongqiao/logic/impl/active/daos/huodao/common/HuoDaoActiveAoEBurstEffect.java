...
/**
 * 火道主动：范围爆发（优先普通伤害）+ 可选附带一个负面效果、轻微击退与点燃。
 * <p>
 * 约束：
 * <ul>
 *   <li>真元消耗必须走标准折算（由 {@link GuEffectCostHelper} 内部调用）</li>
 *   <li>所有效果强度接入火道道痕算法</li>
 *   <li>法术伤害穿甲过强，默认优先普通伤害</li>
 * </ul>
 * </p>
 */
public class HuoDaoActiveAoEBurstEffect implements IGuEffect {
    ...
    public HuoDaoActiveAoEBurstEffect(
        ...
        for (LivingEntity target : user.level().getEntitiesOfClass(
            ...
            if (debuff != null && duration > 0) {
                final int scaledDuration =
                    DaoHenEffectScalingHelper.scaleDurationTicks(duration, multiplier);
                if (scaledDuration > 0) {
                    target.addEffect(
