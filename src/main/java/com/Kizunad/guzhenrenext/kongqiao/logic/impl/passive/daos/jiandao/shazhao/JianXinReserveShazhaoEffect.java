 ...
 * 剑道被动杀招：剑心蓄势。
 * <p>
 * 高转常驻：每秒维持扣费，提升多项资源上限，并缓慢恢复真元；数值随【剑】道痕动态变化。
 * </p>
 */
public class JianXinReserveShazhaoEffect implements IShazhaoEffect {

    public static final String SHAZHAO_ID =
        "guzhenrenext:shazhao_passive_jian_xin_reserve";
    ...
    public void onSecond(final LivingEntity user, final ShazhaoData data) {
        ...
        applyZhenyuanRestore(user, data, selfMultiplier);
    }
    ...
    private static void applyZhenyuanRestore(
        final LivingEntity user,
        final ShazhaoData data,
        final double multiplier
    ) {
        ...
        if (clamped > DEFAULT_AMOUNT) {
            ZhenYuanHelper.modify(user, clamped);
        }
    }
