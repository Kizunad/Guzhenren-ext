package com.Kizunad.guzhenrenext.kongqiao.logic.impl.active.daos.huodao.common;
...
/**
 * 火道主动：锁定目标的单体打击（优先普通伤害）+ 可选附带多个负面效果与点燃。
 */
public class HuoDaoActiveTargetNukeEffect implements IGuEffect {
    ...
    public HuoDaoActiveTargetNukeEffect(
    ...
    public boolean onActivate(
        ...
        applyBurn(target, usageInfo, multiplier);
        applyDamage(user, target, usageInfo, multiplier);
        applySelfHeal(user, usageInfo, multiplier);
    ...
    private void applyBurn(
        ...
        final double burnSecondsBase = Math.max(
            0.0,
            UsageMetadataHelper.getDouble(usageInfo, META_BURN_SECONDS, 0.0)
        );
    ...
    private void applySelfHeal(
        final LivingEntity user,
        final NianTouData.Usage usageInfo,
        final double multiplier
    ) {
        final double selfHealBase = Math.max(
            0.0,
        ...
        final double heal = Math.min(
            MAX_SELF_HEAL,
            DaoHenEffectScalingHelper.scaleValue(selfHealBase, multiplier)
        );
        if (heal > 0.0) {
