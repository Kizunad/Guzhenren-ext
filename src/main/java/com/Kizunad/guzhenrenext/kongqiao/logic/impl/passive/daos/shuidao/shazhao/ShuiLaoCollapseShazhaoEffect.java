 ...
 * 水道主动杀招：水牢崩塌。
 * <p>
 * 视线锁定单体，在目标周围引爆水牢：范围普通伤害 + 牵引 + 迟滞。
 * </p>
 */
public class ShuiLaoCollapseShazhaoEffect implements IShazhaoActiveEffect {

    public static final String SHAZHAO_ID =
        "guzhenrenext:shazhao_active_shui_lao_collapse";
    ...
    private static final double DEFAULT_DAMAGE = 2200.0;
    private static final double DEFAULT_PULL_STRENGTH = 0.35;
    private static final double MIN_VALUE = 0.0;

    private static final double MAX_DAMAGE_PER_TARGET = 20000.0;
    private static final double MAX_PULL_STRENGTH = 1.2;
    private static final double MIN_HORIZONTAL_VECTOR_SQR = 0.0001;
    private static final double VERTICAL_PUSH = 0.05;

    private static final int DEFAULT_SLOW_DURATION_TICKS = 200;
    private static final int DEFAULT_SLOW_AMPLIFIER = 1;
    private static final int DEFAULT_COOLDOWN_TICKS = 1400;
    ...
    public boolean onActivate(final ServerPlayer player, final ShazhaoData data) {
        ...
        final double basePull = ShazhaoMetadataHelper.clamp(
            Math.max(MIN_VALUE, ShazhaoMetadataHelper.getDouble(data, META_PULL_STRENGTH, DEFAULT_PULL_STRENGTH)),
            MIN_VALUE,
            MAX_PULL_STRENGTH
        );
        ...
        for (LivingEntity target : targets) {
            ...
            final double scaledPull = basePull * DaoHenEffectScalingHelper.clampMultiplier(multiplier);
            applyPull(anchor.position(), target, scaledPull);
        }
    ...
    private static void applyPull(
        ...
        final Vec3 delta = center.subtract(target.position());
