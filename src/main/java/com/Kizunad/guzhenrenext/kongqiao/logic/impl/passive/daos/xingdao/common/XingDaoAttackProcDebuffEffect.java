...
/**
 * 星道通用被动：攻击触发（概率）对目标施加 debuff，并可附带额外伤害。
 * <p>
 * - 资源消耗：真元折算 +（念头/精力/魂魄任选其一即可），触发时仅扣一次。
 * - 伤害：优先使用物理伤害（受护甲影响），法术伤害仅作为兼容兜底。
 * - 概率与持续时间：随星道道痕动态变化（倍率裁剪防止失控）。
 * </p>
 */
public class XingDaoAttackProcDebuffEffect implements IGuEffect {
    ...
    public XingDaoAttackProcDebuffEffect(
