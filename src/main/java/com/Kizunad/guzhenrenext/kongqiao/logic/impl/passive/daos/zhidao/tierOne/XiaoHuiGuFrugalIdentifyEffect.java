 ...
 * 一转小慧蛊：被动【省念】。
 * <p>
 * 设计目标：在鉴定进程中，降低每 Tick 念头消耗（由 {@link com.Kizunad.guzhenrenext.kongqiao.handler.NianTouTickHandler}
 * 读取），并通过真元维持成本进行平衡。
 * </p>
 */
public class XiaoHuiGuFrugalIdentifyEffect implements IGuEffect {

    public static final String PASSIVE_USAGE_ID =
        "guzhenren:xiaohuigu_passive_frugal_identify";

    private static final String ITEM_ID = "guzhenren:xiaohuigu";

    private static final double DEFAULT_ZHENYUAN_BASE_COST_PER_SECOND = 3.2;
    private static final double DEFAULT_IDENTIFY_COST_MULTIPLIER = 0.85;
    ...
    public static double getIdentifyCostMultiplierFromConfig() {
        final NianTouData.Usage usage = resolveUsage();
        if (usage == null) {
            return DEFAULT_IDENTIFY_COST_MULTIPLIER;
        }
        final double multiplier = getMetaDouble(
            usage,
            "identify_cost_multiplier",
            DEFAULT_IDENTIFY_COST_MULTIPLIER
        );
        return clampDouble(multiplier, 0.0, 1.0);
    }
