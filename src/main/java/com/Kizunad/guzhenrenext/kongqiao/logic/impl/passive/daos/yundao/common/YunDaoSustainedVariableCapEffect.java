package com.Kizunad.guzhenrenext.kongqiao.logic.impl.passive.daos.yundao.common;
...
/**
 * 云道通用被动：持续性维持（多资源）+ 提升 Guzhenren 字段上限（仅用于高转）。
 * <p>
 * 通过 metadata 配置：
 * <ul>
 ...
 * </p>
 */
public class YunDaoSustainedVariableCapEffect implements IGuEffect {
    ...
    public YunDaoSustainedVariableCapEffect(
    ...
    private void applyCaps(
        ...
        for (CapSpec cap : caps) {
            if (cap == null || cap.variableKey() == null || cap.amountMetaKey() == null) {
                continue;
            }
    ...
    private void clearAll(final LivingEntity user) {
        for (CapSpec cap : caps) {
            if (cap == null || cap.variableKey() == null) {
                continue;
            }
