...
public final class NianTouHelper {
    ...
    public static double getAmount(final LivingEntity entity) {
        ...
        return readDoubleField(getVariables(entity), FIELD_NIANTOU, 0.0);
    }

    public static void modify(final LivingEntity entity, final double amount) {
        if (entity == null) {
            return;
        }
        ...
        final double original = readDoubleField(variables, FIELD_NIANTOU, 0.0);
        final double newValue = Math.max(0, original + amount);

        if (Double.compare(original, newValue) != 0 && writeDoubleField(variables, FIELD_NIANTOU, newValue)) {
