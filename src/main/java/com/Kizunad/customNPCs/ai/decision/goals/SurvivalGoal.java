...
public class SurvivalGoal implements IGoal {
    ...
    public boolean canRun(INpcMind mind, LivingEntity entity) {
        return entity.getHealth() < entity.getMaxHealth() * LOW_HEALTH_THRESHOLD;
    }
    
    @Override
    public void start(INpcMind mind, LivingEntity entity) {
        // 记录进入生存模式
        mind.getMemory().rememberShortTerm("in_survival_mode", true, SURVIVAL_MODE_MEMORY_DURATION);
        
        // FUTURE: 这里可以添加实际的生存行为，如：
        // - 寻找食物
        // - 寻找治疗物品
        // - 逃跑
        System.out.println("[SurvivalGoal] NPC " + entity.getName().getString() 
            + " 进入生存模式！血量: " + entity.getHealth());
    }
    ...
    public void stop(INpcMind mind, LivingEntity entity) {
        ...
        System.out.println("[SurvivalGoal] NPC " + entity.getName().getString() + " 退出生存模式");
    }
