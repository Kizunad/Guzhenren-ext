...
public class VisionSensor implements ISensor {
    ...
    public void sense(INpcMind mind, LivingEntity entity, ServerLevel level) {
        ...
        if (nearbyEntities.isEmpty()) {
            mind.getMemory().rememberShortTerm("visible_entities_count", 0, MEMORY_DURATION);
            return;
        }
        ...
        if (!visibleEntities.isEmpty()) {
            // 找到最近的实体
            LivingEntity nearest = visibleEntities.stream()
            ...
            if (nearest != null) {
                // 存储最近实体的信息
                mind.getMemory().rememberShortTerm("nearest_entity", 
                    nearest.getUUID().toString(), MEMORY_DURATION);
                mind.getMemory().rememberShortTerm("nearest_entity_type", 
                    nearest.getType().toString(), MEMORY_DURATION);
                mind.getMemory().rememberShortTerm("nearest_entity_distance", 
                    entity.distanceTo(nearest), MEMORY_DURATION);
            }
        }
    }
     ...
     * @return 是否能看到
     */
    private boolean hasLineOfSight(LivingEntity observer, Entity target, ServerLevel level) {
