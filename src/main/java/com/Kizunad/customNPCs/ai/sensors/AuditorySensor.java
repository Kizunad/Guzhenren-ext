...
public class AuditorySensor implements ISensor {
    ...
    private void updateMemory(INpcMind mind, List<SoundEvent> soundEvents) {
        ...
        // 存储声音事件列表（限制数量）
        List<SoundEvent> limitedEvents = soundEvents
            .stream()
        ...
        // 找到最响亮的声音
        SoundEvent loudest = soundEvents
            .stream()
        ...
        if (loudest != null) {
            ...
            mind
                .getMemory()
                .rememberShortTerm(
                    "loudest_sound_source",
                    loudest.sourceEntity().toString(),
                    MEMORY_DURATION
                );
            mind
                .getMemory()
                .rememberShortTerm(
                    "loudest_sound_source_type",
