...
public class MindDebugCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("npc_mind")
    ...
    private static int inspectLookingAt(CommandContext<CommandSourceStack> context) {
        try {
            Entity entity = context.getSource().getEntityOrException();
            if (!(entity instanceof LivingEntity player)) {
                context.getSource().sendFailure(Component.literal("只有实体可以执行此命令"));
                return 0;
            }
            ...
            Vec3 endPosition = eyePosition.add(viewVector.scale(RAYTRACE_DISTANCE));
            ...
            context.getSource().sendFailure(Component.literal("未找到目标实体"));
            return 0;
            
        } catch (Exception e) {
            context.getSource().sendFailure(Component.literal("错误: " + e.getMessage()));
            return 0;
        }
    }
    ...
    private static int printMindInfo(CommandSourceStack source, Entity entity) {
        if (!(entity instanceof LivingEntity livingEntity)) {
            source.sendFailure(Component.literal("目标不是生物: " + entity.getName().getString()));
            return 0;
        }
        
        if (!livingEntity.hasData(NpcMindAttachment.NPC_MIND)) {
            source.sendFailure(Component.literal("目标没有思维系统: " + entity.getName().getString()));
            return 0;
        }
        
        INpcMind mind = livingEntity.getData(NpcMindAttachment.NPC_MIND);
        
        source.sendSuccess(() -> Component.literal("=== NPC Mind Info: " + entity.getName().getString() + " ==="),
            false);
        
        // 1. 当前目标
        var currentGoal = mind.getGoalSelector().getCurrentGoal();
        if (currentGoal != null) {
            source.sendSuccess(() -> Component.literal("当前目标: " + currentGoal.getName()), false);
            source.sendSuccess(() -> Component.literal("优先级: " + currentGoal.getPriority(mind, livingEntity)), false);
        } else {
            source.sendSuccess(() -> Component.literal("当前目标: (无)"), false);
        }
        ...
        if (!mind.getMemory().getShortTermMemory().isEmpty()) {
            ...
            mind.getMemory().getShortTermMemory().forEach((key, entry) -> {
                source.sendSuccess(() -> Component.literal("    - " + key + ": " + entry.getValue()), false);
            });
        }
        
        if (!mind.getMemory().getLongTermMemory().isEmpty()) {
            source.sendSuccess(() -> Component.literal("  长期记忆:"), false);
            mind.getMemory().getLongTermMemory().forEach((key, entry) -> {
                source.sendSuccess(() -> Component.literal("    - " + key + ": " + entry.getValue()), false);
            });
        }
