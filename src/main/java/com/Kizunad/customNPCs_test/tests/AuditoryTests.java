...
public class AuditoryTests {
    ...
    public static void testAuditorySensorDetectsMovement(GameTestHelper helper) {
        // 创建观察者（静止）
        Zombie observer = TestEntityFactory.createTestZombie(helper, new BlockPos(2, 2, 2));
        ...
        NpcTestHelper.waitForAssertion(helper, () -> {
            if (!mind.getMemory().hasMemory("heard_sounds_count")) {
                throw new GameTestAssertException("Auditory sensor memory not present");
            }
            
            int count = mind.getMemory().getShortTerm("heard_sounds_count", Integer.class, -1);
            if (count <= 0) {
                throw new GameTestAssertException("Expected to hear sounds, but count was " + count);
            }
            
            // 验证听到了移动声音
            @SuppressWarnings("unchecked")
            List<String> soundTypes = (List<String>) mind.getMemory()
                .getShortTerm("loudest_sound_types", List.class, List.of());
            if (!soundTypes.contains("movement")) {
                throw new GameTestAssertException("Expected to hear 'movement' sound, but got: " + soundTypes);
            }
    ...
    public static void testAuditorySensorNoSounds(GameTestHelper helper) {
        ...
        NpcTestHelper.waitForAssertion(helper, () -> {
            ...
            int count = mind.getMemory().getShortTerm("heard_sounds_count", Integer.class, -1);
            if (count != 0) {
                throw new GameTestAssertException("Expected 0 sounds, but found " + count);
            }
     ...
     */
    @GameTest(template = "empty")
    public static void testAuditorySensorMultipleSoundTypes(GameTestHelper helper) {
        // 创建观察者
        Zombie observer = TestEntityFactory.createTestZombie(helper, new BlockPos(2, 2, 2));
        INpcMind mind = NpcTestHelper.getMind(helper, observer);
        ...
        NpcTestHelper.waitForAssertion(helper, () -> {
            ...
            if (soundTypes.size() < 2) {
                throw new GameTestAssertException(
                    "Expected to hear multiple sound types, but got: " + soundTypes);
            }
            
            if (!soundTypes.contains("movement")) {
                throw new GameTestAssertException("Expected to hear 'movement' sound");
            }
            
            if (!soundTypes.contains("pain")) {
                throw new GameTestAssertException("Expected to hear 'pain' sound");
            }
     ...
     */
    @GameTest(template = "empty")
    public static void testAuditorySensorDistanceAttenuation(GameTestHelper helper) {
        // 创建观察者
        Zombie observer = TestEntityFactory.createTestZombie(helper, new BlockPos(5, 2, 5));
        INpcMind mind = NpcTestHelper.getMind(helper, observer);
        ...
        NpcTestHelper.waitForCondition(helper, 
            ...
            50, "Should detect near sound");
        
        double nearIntensity = mind.getMemory().getShortTerm("loudest_sound_intensity", Double.class, 0.0);
        ...
        helper.runAfterDelay(TICK_DELAY, () -> {
            ...
            // 验证远距离声音强度更低
            helper.runAfterDelay(TICK_DELAY, () -> {
                double farIntensity = mind.getMemory().getShortTerm("loudest_sound_intensity", Double.class, 0.0);
                
                if (farIntensity >= nearIntensity) {
                    helper.fail("Far sound intensity (" + farIntensity + 
                        ") should be less than near sound intensity (" + nearIntensity + ")");
                }
     ...
     */
    @GameTest(template = "empty")
    public static void testAuditorySensorMultipleSources(GameTestHelper helper) {
        // 创建观察者
        Zombie observer = TestEntityFactory.createTestZombie(helper, new BlockPos(5, 2, 5));
        INpcMind mind = NpcTestHelper.getMind(helper, observer);
        ...
        // 验证：选择了最响亮（近距离）的声音
        NpcTestHelper.waitForAssertion(helper, () -> {
            int count = mind.getMemory().getShortTerm("heard_sounds_count", Integer.class, 0);
            if (count < 2) {
                throw new GameTestAssertException("Expected to hear at least 2 sounds, but got " + count);
            }
            
            // 最响亮的声音应该来自近距离目标
            Vec3 loudestLocation = mind.getMemory().getShortTerm("loudest_sound_location", Vec3.class, Vec3.ZERO);
            double distanceToLoudest = loudestLocation.distanceTo(observer.position());
            double distanceToNear = nearTarget.position().distanceTo(observer.position());
