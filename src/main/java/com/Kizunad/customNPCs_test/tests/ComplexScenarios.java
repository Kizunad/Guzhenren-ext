...
public class ComplexScenarios {
    ...
    public static void testTheGatherer(GameTestHelper helper) {
        ...
        System.out.println("[testTheGatherer] 场景设置完成: Zombie在 " + 
        ...
        if (zombie.hasData(NpcMindAttachment.NPC_MIND)) {
            ...
            System.out.println("[testTheGatherer] NpcMind已配置，目标优先级: " + 
                gatherGoal.getPriority(mind, zombie));
        }
        ...
        helper.succeedWhen(() -> {
            ...
            if (!itemEntity.isAlive()) {
                ...
                helper.fail("物品实体仍存在，未被拾取");
            }
            
            // 验证 Zombie 主手持有物品
            ItemStack heldItem = zombie.getItemInHand(InteractionHand.MAIN_HAND);
            if (!heldItem.isEmpty() && heldItem.getItem() == Items.STICK) {
                System.out.println("[testTheGatherer] ✓ Zombie主手持有木棍 x" + heldItem.getCount());
            } else {
                helper.fail("Zombie主手应该持有木棍，但实际为: " + 
                    (heldItem.isEmpty() ? "空" : heldItem.getHoverName().getString()));
            }
            
            System.out.println("[testTheGatherer] ===== 测试通过！=====");
        });
        
        // 4. 在每个 tick 驱动 NpcMind
        helper.onEachTick(() -> {
            if (zombie.hasData(NpcMindAttachment.NPC_MIND)) {
                var mind = zombie.getData(NpcMindAttachment.NPC_MIND);
                mind.tick(helper.getLevel(), zombie);
            }
        });
        
        // 设置最大等待时间 100 ticks (5秒)
        helper.succeedOnTickWhen(100, () -> {
            // 提前成功检查
            if (!itemEntity.isAlive() && 
                !zombie.getItemInHand(InteractionHand.MAIN_HAND).isEmpty()) {
                System.out.println("[testTheGatherer] 提前完成任务！");
            }
        });
    }
    ...
    public static void testTheCourier(GameTestHelper helper) {
        ...
        if (zombie.hasData(NpcMindAttachment.NPC_MIND)) {
            ...
            // 添加 CourierGoal
            CourierGoal courierGoal = new CourierGoal(itemEntity, targetPos, 100.0f);
        ...
        helper.succeedWhen(() -> {
            ...
            if (heldItem.isEmpty()) {
                ...
                helper.fail("Zombie主手应该为空，但持有: " + heldItem.getHoverName().getString());
            }
            
            // 验证目标位置附近存在物品实体
            AABB searchBox = new AABB(targetPos).inflate(3.0);
        ...
        // 设置最大等待时间 200 ticks (10秒)
        helper.succeedOnTickWhen(200, () -> {
            // 提前成功检查
            AABB searchBox = new AABB(targetPos).inflate(3.0);
            List<ItemEntity> itemsAtTarget = helper.getLevel().getEntitiesOfClass(
                ItemEntity.class,
            ...
            if (!itemsAtTarget.isEmpty() && 
                zombie.getItemInHand(InteractionHand.MAIN_HAND).isEmpty()) {
                System.out.println("[testTheCourier] 提前完成任务！");
            }
        });
    }
    ...
    public static void testPerformanceStress(GameTestHelper helper) {
        ...
        for (int i = 0; i < 50; i++) {
            ...
            helper.spawn(EntityType.VILLAGER, pos);
        }
        ...
        // 生成测试 NPC
        Zombie testNpc = helper.spawn(EntityType.ZOMBIE, centerPos);
        ...
        // 5. 验证性能指标
        helper.succeedOnTickWhen(sampleSize + 5, () -> {
            long planningTimeMs = (planningEndTime[0] - planningStartTime[0]) / 1_000_000;
