LibSourceCodes 可以参考 Neoforge Source Code / guzhenren Decompile Code / chestcavity Source Code / Minecraft 1.21.1-Deobfuscated
必须遵循：
- 使用 Serena MCP - Project: Guzhenren-ext
- 使用 `agent_workflow.py assign` 时务必附带 `--details` 描述当前工作内容，便于多 AGENT 识别与避免冲突；同一项目若已被占用需先确认完成并执行 `unlock` 释放。
- 流程遵循 - 了解上下文(查看代码，任务) -> （可选/用户要求时）反馈了解情况 -> 计划任务 -> 具体编写(此处 assign project) -> 代码规范测试/编译测试 -> 测试gameTest编写&测试 -> 代码提交，unlock project -> 反馈用户 (提醒: Task过多可多流程进行)
- ./gradlew checkstyleMain 检查代码是否符合规范，禁止@suppresswarnings 这种违规操作 
- 注释：中文，详细
可选遵循：
- 当用户要求不够清晰，可选通过反问方法 - 配置一个 checklist.md 了解用户的具体需求，可提供多个选择，并且详
  细解释优缺点等细节
